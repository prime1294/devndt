@extends('admin.v1.layout.app', ['title' => 'Edit Process'])
...
<section class="content-header">
  <h1>
    <i class="fa fa-cubes"></i> Edit Process
    <small>Edit Stock Process</small>
  </h1>
  <ol class="breadcrumb">
...
<section class="content">
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="box-body">
        <form method="post" action="{{ route('new.process.update',$info->id) }}" enctype="multipart/form-data">
          {!! csrf_field() !!}
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                <label>Payment Type</label>
                <input type="checkbox" value="1" id="confirmAns" {{ $info->type == 1 ? "checked" : ""  }} name="confirmAns" class="confirmAns" data-size="mini" data-toggle="toggle" data-on="Cash" data-off="Credit" data-onstyle="success" data-offstyle="info">
              </div>
              </div>
            </div>
          <div class="row">
              ...
              <div class="col-md-4 due_date_div">
                ...
                <div class="col-md-4 process_name_div">
                  <div class="form-group">
                        ...
                        <select id="process_name" name="process_name" class="form-control select2-bank" data-placeholder="Select Processor">
                          <option value="">Select Processor</option>
                          <option data-img="{{ 'plus.png' }}" value="0">Add New</option>
                          @foreach($process_list as $row)
                          <option data-state="{{ $row->state  }}" data-img="{{ $row->photo }}" value="{{ $row->id }}" {{ $info->process_id == $row->id ? "selected" : ""  }}>{{ ucwords($row->name) }}</option>
                          @endforeach
                        </select>
                        </div>
                    </div>
          ...
          <div class="row">
            <div class="col-md-4">
              <div class="form-group">
                    ...
                    <div class="input-group">
                    <span class="input-group-addon"><i class="glyphicon glyphicon-lock"></i></span>
                    <select id="type_of_manufacturer" multiple="multiple" name="type_of_manufacturer[]" class="form-control select2">
                      <?php
                       $exp_data = !empty($info->manufacturer_type) ? explode(',',$info->manufacturer_type) : [];
                      foreach($manufacturer as $row) {
                        ?>
                        <option value="{{ $row->id }}" {{ in_array($row->id, $exp_data) ? "selected" : ""  }} {{ Admin::find_in_set(@$pinfo->types_of_menufecture,$row->id) ? "selected" : "" }}>{{ $row->name }}</option>
                        <?php
                      }
                      ?>
                    </select>
            ...
            <div class="col-md-4">
              <div class="form-group">
                    <label for="ref_img">Sample Photo <?= $info->sample != "" ? '<a href="'.asset($info->sample).'" target="_blank">View Current</a>' : "" ?></label>
                    ...
                    <input type="hidden" id="upload_image_text" value="{{ $info->sample }}" name="upload_image_text">
                </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
                ...
                <div class="input-group">
                <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                <input type="text" id="expected_date" name="expected_date" value="{{ $info->expected_date != "1970-01-01" ? date('d-m-Y',strtotime($info->expected_date))  : "" }}" class="form-control datepicker">
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="row">
              ...
              <div class="form-group">
                ...
                <div class="input-group">
                  ...
                  <select  style="width: 100%;" id="process_state" name="process_state" class="form-control select2">
                    ...
                    ?>
                    <option value="{{ $row->state_id }}" data-gst="{{ $row->gst_code }}" {{ $info->process_state == $row->state_id ? "selected" : "" }}>{{ $row->state }}</option>
                    <?php
                    }
                    ?>
                  </select>
            ...
            <div class="col-md-4">
              <div class="form-group">
                ...
                <div class="input-group">
                  <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                  <input type="text" id="process_transport" name="process_transport" value="{{ $info->process_transport }}" class="form-control">
                </div>
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
                <label for="imagebtnbill">Bill Photo <?= $info->bill_photo != "" ? '<a href="'.asset($info->bill_photo).'" target="_blank">View Current</a>' : "" ?></label>
                ...
                <input type="hidden" id="upload_image_text_bill" value="{{ $info->bill_photo }}" name="upload_image_text_bill">
              </div>
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="table-responsive">
              <table class="table table-hover table-striped mytbl fixtablemobile2">
                ...
                <?php
                $no_new = 1;
                        if($process_item->isNotEmpty()) {
                    foreach($process_item as $re) {
                      $is_lock = $re->col_lock ? "disabled" : "";
                ?>
                  <tr class="tblrow{{ $no_new }}" data-id="{{ $no_new++ }}">
                    <td class="ele1">
                      <div class="form-group">
                      <select style="width: 100%;" id="stock_no" name="stock_no[]" class="form-control stock_no select2" {{ $is_lock }}>
                        <option value="">Select Stock</option>
                        @foreach($stock_list as $row)
                        <option value="{{ $row->id }}" {{ $re->stock_id == $row->id ? "selected" : "" }} data-pending="{{ $row->pending }}" data-unit="{{ $row->unit }}">{{ Admin::FormateStockItemID($row->id) }} </option>
                        @endforeach
                      </select>
                      </div>
                    </td>
                    <td class="ele2">
                      <div class="form-group">
                      <input type="text" {{ $is_lock }} id="description" value="{{ $re->description }}" name="description[]" class="form-control description text-center" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele3">
                      <div class="form-group">
                      <input type="text" {{ $is_lock }} id="design_name" value="{{ $re->design_name }}"  name="design_name[]" class="form-control design_name text-center" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele4">
                      <div class="form-group">
                      <input type="text" {{ $is_lock }} id="quantity" value="{{ $re->quantity }}"  name="quantity[]" class="form-control quantity checkcalculation onlyint text-center" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele11">
                      <div class="form-group">
                      <select id="mesurement" style="width:100%;" name="mesurement[]" class="form-control mesurement select2" {{ $is_lock }}>
                        @foreach($category_list as $row)
                        <option value="{{ $row->id }}" {{ $row->id == $re->mesurement ? "selected" : "" }}>{{ $row->name }}</option>
                        @endforeach
                      </select>
                      </div>
                    </td>
                    <td class="ele5">
                      <div class="form-group">
                      <input type="text" {{ $is_lock }} id="unit" name="unit[]"  class="form-control unit onlyint text-center checkcalculation" value="{{ $re->rate }}" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele6">
                      <div class="form-group">
                      <input type="text" {{ $is_lock }} id="discount" name="discount[]" class="form-control discount onlyint text-center checkcalculation" maxlength='2' value="{{ $re->discount }}" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele7">
                      <div class="form-group">
                      <input type="text" id="discount_amount" name="discount_amount[]" readonly class="form-control discount_amount onlyint text-center" value="{{ $re->discount_amount }}" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele8">
                      <div class="form-group">
                      <input type="text" {{ $is_lock }} id="gst" name="gst[]" class="form-control gst onlyint text-center checkcalculation" maxlength='2' value="{{ $re->gst }}" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele9">
                      <div class="form-group">
                      <input type="text" id="gst_amount" name="gst_amount[]" readonly class="form-control gst_amount onlyint text-center" value="{{ $re->gst_amount }}" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele10">
                      <div class="form-group">
                      <input type="text" {{ $is_lock }} id="total" name="total[]" readonly class="form-control total onlyint text-center checkcalculation" value="{{ $re->total }}" autocomplete="false">
                      </div>
                    </td>
                    <td class="ele13">
                      @if($re->col_lock)
                      <button type="button" class="btn btn-danger btn-xs" onclick="toastr.error('You can not edit or delete this process. you need to delete all received quantity first')"><i class="fa fa-trash"></i></button>
                      @else
                ...
                <tr class="tblrow1" data-id="1">
                  ...
                  <td class="ele4">
                    <div class="form-group">
                      <input type="text" id="quantity" name="quantity[]" class="form-control quantity onlyint checkcalculation text-center" autocomplete="false">
                    </div>
                  </td>
                  <td class="ele11">
                    <div class="form-group">
            ...
            <div class="col-md-6 form-inline">
              ...
              <div class="form-group pull-right" style="margin-top: 15px;">
                ...
                <div class="input-group">
                  <span class="input-group-addon"><i class="fa fa-inr"></i></span>
                  <input type="text" class="form-control onlyint text-center" value="{{ $info->less_amount }}" id="less_total" name="less_total">
                </div>
              </div>
                </div>
                <div class="col-md-12">
              <div class="form-group pull-right" style="margin-top: 15px;">
                ...
                <div class="input-group">
                  <span class="input-group-addon"><i class="fa fa-inr"></i></span>
                  <input type="text" class="form-control onlyint text-center" readonly value="{{ $info->final_total }}" id="final_total" name="final_total">
                </div>
              </div>
                </div>
              </div>
          ...
          <div class="table-responsive">
            <table class="table table-hover table-striped table-bordered fixtablemobile">
              ...
              @if(!$process_payment->isEmpty())
                @foreach($process_payment as $row)
                  <tr data-id="{{ $row->id  }}">
                    ...
                    <td><input type="text" id="sp_ref_no" name="sp_ref_no[]" value="{{ $row->ref_no }}" class="form-control sp_ref_no"></td>
...
$(document).on("click",".removerowpayment",function(e) {
  ...
  if(ele.attr('data-id')) {
    ...
    if(conf) {
      var route = '{{ route('delete.process.payment',":ID") }}';
