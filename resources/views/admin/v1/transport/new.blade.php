@extends('admin.v1.layout.app', ['title' => 'Add Transport'])
...
<section class="content-header">
  <h1>
    <i class="fa fa-user"></i> Transport
    <?php if($type == "edit") {
      ?><small>Edit Transport</small><?php
    } else {
      ?><small>Add New Transport</small><?php
    } ?>
  </h1>
  <ol class="breadcrumb">
    <li><a href="{{ route('user.dashboard') }}"><i class="fa fa-dashboard"></i> Home</a></li>
    <?php if($type == "edit") {
      ?><li class="active">Edit Transport</li><?php
    } else {
      ?><li class="active">Add New Transport</li><?php
    } ?>
...
<section class="content">
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="box-body">
        <?php if($type == "edit") {  ?>
        <form method="post" action="{{ route('transport.update',[$pinfo->id]) }}" enctype="multipart/form-data">
        <?php } else { ?>
        <form method="post" action="{{ route('transport.register') }}" enctype="multipart/form-data">
        <?php } ?>
          {!! csrf_field() !!}
            <div class="row">
              <div class="col-md-12">
            ...
            <div class="row">
              <div class="col-md-4">
                <div class="form-group">
                      <label for="person_name">Transport Person Name</label>
              ...
              <div class="col-md-4">
                <div class="form-group">
                      <label for="party_name">Vehical Type</label>
                      <div class="input-group">
                      <span class="input-group-addon"><i class="glyphicon glyphicon-lock"></i></span>
                      <select id="party_name" name="party_name" class="form-control select2">
                        <option value="Loading" {{ @$pinfo->business_name == "Loading" ? "selected" : "" }}>Loading</option>
                        <option value="Auto-Ricsha" {{ @$pinfo->business_name == "Auto-Ricsha" ? "selected" : "" }}>Auto-Ricsha</option>
                        <option value="Chota-Hathi" {{ @$pinfo->business_name == "Chota-Hathi" ? "selected" : "" }}>Chota-Hathi</option>
                        <option value="Eicher" {{ @$pinfo->business_name == "Eicher" ? "selected" : "" }}>Eicher</option>
                      </select>
                      </div>
                  </div>
              </div>
              <div class="col-md-4">
                <div class="form-group">
                      <label for="vehical_no">Vehical Number</label>
                      <div class="input-group">
                      <span class="input-group-addon"><i class="glyphicon glyphicon-lock"></i></span>
                      <input type="text" id="vehical_no" name="vehical_no" class="form-control" onkeyup="this.value = this.value.toUpperCase();" value="{{ @$pinfo->vehical_no }}">
                      </div>
                  </div>
              </div>
            </div>
            ...
            <div class="row">
              ...
              <div class="col-md-4">
                <div class="form-group">
                      ...
                      <div class="input-group">
                      <span class="input-group-addon"><i class="glyphicon glyphicon-lock"></i></span>
                      <input type="text" id="bank_ifsc" name="bank_ifsc" class="form-control" value="{{ @$pinfo->ifsc_code }}">
                      </div>
                  </div>
              </div>
              <div class="col-md-4">
...
<div class="buy-now">
<a href="{{ route('transport') }}" class="btn btn-primary buy-now-btn">
...
function val_add_party()
{
  ...
  if($("#person_name").val() == "")
	{
		toastr.error("Please, Enter Transport Person Name");
		$("#person_name").focus();
		return false;
	}
  if($("#vehical_no").val() == "")
	{
		toastr.error("Please, Enter vehical number");
		$("#vehical_no").focus();
		return false;
	}
  if($("#email").val() != "") {
