@extends('admin.v1.layout.app', ['title' => 'Transport'])
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="box-header with-border">
        <h3 class="box-title">List of Transport</h3>
...
<div class="buy-now">
<a href="{{ route('transport.new') }}" class="btn btn-primary buy-now-btn">
...
$(document).on("change",".status_checkbox",function(e){
  // alert('hello');
	...
	var jqxhr = $.ajax({
    url:'{{ route("transport.activation") }}',
    ...
    type:'POST'
	  ...
	  .done(function(e) {
		if(e)
		{
			toastr.success("Transport "+status_txt+" Successfully.");
		}
		else
		{
			toastr.error('Ooops..! Something went wrong');
...
$(document).ready(function() {
  table = $('.dtable').DataTable({
    ...
    "serverSide" : true,
    "order" : [],
    "ajax" : "{{ route('transport.list.ajax') }}",
    "columns" : [
      {"data":"party"},
...
var user_id = data.id;
if(user_id) {
  var route = '{{ route('transport.view',":ID") }}';
