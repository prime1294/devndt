@extends('admin.v1.layout.app', ['title' => 'Add Karigar'])
...
<section class="content-header">
  <h1>
    <i class="fa fa-user"></i> Karigar
    <?php if($type == "edit") {
      ?><small>Edit Karigar</small><?php
    } else {
      ?><small>Add New Karigar</small><?php
    } ?>
  </h1>
  <ol class="breadcrumb">
    <li><a href="{{ route('user.dashboard') }}"><i class="fa fa-dashboard"></i> Home</a></li>
    <?php if($type == "edit") {
      ?><li class="active">Edit Karigar</li><?php
    } else {
      ?><li class="active">Add New Karigar</li><?php
    } ?>
...
<section class="content">
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="box-body">
        <?php if($type == "edit") {  ?>
        <form method="post" action="{{ route('karigar.update',[$pinfo->id]) }}" enctype="multipart/form-data">
        <?php } else { ?>
        <form method="post" action="{{ route('karigar.register') }}" enctype="multipart/form-data">
        <?php } ?>
          {!! csrf_field() !!}
            <div class="row">
              <div class="col-md-12">
                <!-- <center> -->
                  <div class="fbpickermain">
                      <div class=fbpiker>
                          ...
                          <img id="fbholdernew" data-default="{{ @$pinfo->photo ? asset($pinfo->photo) : asset('user_404.jpg') }}" src="{{ @$pinfo->photo ? asset($pinfo->photo) : asset('user_404.jpg') }}"  onclick="triggerfile('fbholdernew','fbinputtxt','image/karigar/','.jpg,.png,.jpeg','box')">
                      </div>
            ...
            <div class="row">
              <div class="col-md-4">
                <div class="form-group">
                      <label for="person_name">Karigar Name <?= COMPICON ?></label>
              ...
              <div class="col-md-4">
                <div class="form-group">
                      <label for="person_salary">Karigar Salary / Month <?= COMPICON ?></label>
                      <div class="input-group">
                      <span class="input-group-addon"><i class="fa fa-inr"></i></span>
                      <input type="text" id="person_salary" name="person_salary" class="form-control onlyint" value="{{ @$pinfo->business_name }}">
                      </div>
                  </div>
              </div>
            <div class="col-md-4">
...
function val_add_party()
{
  if($("#person_name").val() == "")
	{
		toastr.error("Please, Enter Karigar Name");
		$("#person_name").focus();
		return false;
	}
  if($("#person_salary").val() == "")
	{
		toastr.error("Please, Enter Karigar Salary");
		$("#person_salary").focus();
		return false;
	}
}
