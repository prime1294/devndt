@extends('admin.v1.layout.app', ['title' => 'Edit Bank Adjustment'])
...
<section class="content-header">
  <h1>
    <i class="fa fa-sliders"></i> Edit Bank Adjustment
    <small>Adjust Your Bank Account</small>
  </h1>
  <ol class="breadcrumb">
    <li><a href="{{ route('user.dashboard') }}"><i class="fa fa-dashboard"></i> Home</a></li>
    <li class="active">Edit Bank Adjustment</li>
...
<section class="content">
  <div class="row">
    <div class="col-md-12">
      ...
      <div class="box-body">
        <form method="post" action="{{ route('bank.adjustment.update',$info->id) }}" enctype="multipart/form-data">
          {!! csrf_field() !!}
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                    ...
                    <select id="bank_account" name="bank_account" class="form-control select2-bank">
                      ...
                      foreach($bank_list as $row) {
                        ?>
                        <option data-img="{{ $row->bankicon }}" value="{{ $row->id }}" {{ $frombank == $row->id ? "selected" : "" }}>{{ $row->name }} - {{ $row->account_no }} - {{ $row->type == 2 ? "Current" : "Saving" }}</option>
                        <?php
                      }
                      ?>
                    </select>
            ...
            <div class="col-md-4">
              <div class="form-group">
                    ...
                    <select id="entry_type" name="entry_type" class="form-control select2">
                      <option value="bank4" {{ $info->type == "bank4" ? "selected" : "" }}>{{ config('transection.bank4')['type'] }}</option>
                      <option value="bank5" {{ $info->type == "bank5" ? "selected" : "" }}>{{ config('transection.bank5')['type'] }}</option>
                      <option value="bank6" {{ $info->type == "bank6" ? "selected" : "" }}>{{ config('transection.bank6')['type'] }}</option>
                      <option value="bank7" {{ $info->type == "bank7" ? "selected" : "" }}>{{ config('transection.bank7')['type'] }}</option>
                      <?php
                      if(count($bank_list) >= 2) {
                        ?>
                      <option value="bank8" {{ $info->type == "bank8" ? "selected" : "" }}>{{ config('transection.bank8')['type'] }}</option>
                        <?php
                      }
                      ?>
                    </select>
          ...
          <div class="form-group">
          <button type="submit" class="btn btn-primary" onclick="return val_add_party();"><i class="fa fa-upload"></i> Update</button>
          </div>
          </form>
      </div>
      <!-- /.box-body -->
